...
/**
 * Class WritgoCMS_Keyword_Research_Page
 */
class WritgoCMS_Keyword_Research_Page {
	 ...
	 * Instance
	 *
	 * @var WritgoCMS_Keyword_Research_Page
	 */
	 ...
	 * Get instance
	 *
	 * @return WritgoCMS_Keyword_Research_Page
	 */
	...
	public function enqueue_scripts( $hook ) {
		...
		wp_enqueue_style(
			'writgocms-keyword-research',
			WRITGOCMS_URL . 'assets/css/keyword-research.css',
			array(),
			WRITGOCMS_VERSION
		);
		...
		wp_enqueue_script(
			'writgocms-keyword-research',
			WRITGOCMS_URL . 'assets/js/keyword-research.js',
			array( 'jquery' ),
			WRITGOCMS_VERSION,
			true
		);
		...
		wp_localize_script(
			...
			array(
				...
				'nonce'   => wp_create_nonce( 'writgocms_keyword_nonce' ),
				'i18n'    => array(
					'searching'  => __( 'Searching...', 'writgocms' ),
					'error'      => __( 'Error', 'writgocms' ),
					'success'    => __( 'Success', 'writgocms' ),
					'noResults'  => __( 'No results found', 'writgocms' ),
					'saved'      => __( 'Keyword saved successfully', 'writgocms' ),
				),
			)
		);
	}
	...
	public function render_page() {
		...
		<div class="wrap writgocms-keyword-research">
			<h1><?php esc_html_e( 'Keyword Research', 'writgocms' ); ?></h1>
			<p class="description">
				<?php esc_html_e( 'Search for keywords to analyze their search volume, difficulty, and competition.', 'writgocms' ); ?>
			</p>
			...
			<div class="keyword-search-form">
				<div class="search-input-wrapper">
					<input 
						...
						class="regular-text" 
						placeholder="<?php esc_attr_e( 'Enter keyword...', 'writgocms' ); ?>"
					/>
					<button type="button" id="keyword-search-btn" class="button button-primary">
						🔍 <?php esc_html_e( 'Search', 'writgocms' ); ?>
					</button>
				</div>
				<p class="credits-info">
					<?php esc_html_e( 'Cost: 15 credits per search', 'writgocms' ); ?>
				</p>
			</div>
			...
			<div id="keyword-loading" class="keyword-loading" style="display: none;">
				...
				<p><?php esc_html_e( 'Searching...', 'writgocms' ); ?></p>
			</div>
				...
				<div class="keyword-card">
					<div class="keyword-header">
						...
						<button type="button" id="save-keyword-btn" class="button button-secondary">
							💾 <?php esc_html_e( 'Save to Plan', 'writgocms' ); ?>
						</button>
					</div>

					<div class="keyword-metrics">
						<div class="metric-item">
							<span class="metric-label"><?php esc_html_e( 'Search Volume', 'writgocms' ); ?></span>
							<span class="metric-value" id="result-volume">-</span>
							<span class="metric-unit">/month</span>
						</div>
						<div class="metric-item">
							<span class="metric-label"><?php esc_html_e( 'Difficulty', 'writgocms' ); ?></span>
							<span class="metric-value" id="result-difficulty">-</span>
							<span class="metric-badge" id="result-difficulty-badge"></span>
						</div>
						<div class="metric-item">
							<span class="metric-label"><?php esc_html_e( 'CPC', 'writgocms' ); ?></span>
							<span class="metric-value" id="result-cpc">-</span>
						</div>
						<div class="metric-item">
							<span class="metric-label"><?php esc_html_e( 'Competition', 'writgocms' ); ?></span>
					...
					<div class="related-keywords-section">
						<div class="section-header">
							<h3><?php esc_html_e( 'Related Keywords', 'writgocms' ); ?></h3>
							<button type="button" id="load-related-btn" class="button button-small">
								<?php esc_html_e( 'Load Related Keywords', 'writgocms' ); ?> (5 credits)
							</button>
						</div>
					...
					<div class="keyword-actions">
						<button type="button" id="view-serp-btn" class="button">
							📊 <?php esc_html_e( 'View SERP', 'writgocms' ); ?> (10 credits)
						</button>
					</div>
				</div>
			</div>
			...
			<div id="serp-results" class="serp-results" style="display: none;">
				<h3><?php esc_html_e( 'Top 10 SERP Results', 'writgocms' ); ?></h3>
			...
			<div class="saved-keywords-section">
				<h2><?php esc_html_e( 'Saved Keywords', 'writgocms' ); ?></h2>
	...
	private function render_saved_keywords() {
		...
		if ( empty( $keywords ) ) {
			echo '<p>' . esc_html__( 'No saved keywords yet.', 'writgocms' ) . '</p>';
			return;
		}
		...
		echo '<thead>';
		echo '<tr>';
		echo '<th>' . esc_html__( 'Keyword', 'writgocms' ) . '</th>';
		echo '<th>' . esc_html__( 'Volume', 'writgocms' ) . '</th>';
		echo '<th>' . esc_html__( 'Difficulty', 'writgocms' ) . '</th>';
		echo '<th>' . esc_html__( 'CPC', 'writgocms' ) . '</th>';
		echo '<th>' . esc_html__( 'Competition', 'writgocms' ) . '</th>';
		echo '<th>' . esc_html__( 'Saved', 'writgocms' ) . '</th>';
		echo '</tr>';
		echo '</thead>';
		echo '<tbody>';
		...
		foreach ( $keywords as $keyword ) {
			...
			echo '<td>' . esc_html( human_time_diff( strtotime( $keyword['created_at'] ) ) ) . ' ' . esc_html__( 'ago', 'writgocms' ) . '</td>';
			echo '</tr>';
		}
...
// Initialize.
WritgoCMS_Keyword_Research_Page::get_instance();
